...
def trigger_ai_if_needed(game_id, db):
    ...
    with ai_move_locks_lock:
        if ai_move_locks.get(game_id, False):
            print(f"[AI] game={game_id} skipped: lock held")
            return
        ai_move_locks[game_id] = True
    
    def release_lock():
        with ai_move_locks_lock:
            ai_move_locks[game_id] = False
    ...
    try:
        ...
        # 捕获任何未处理的异常，确保锁被释放
        print(f"[AI] game={game_id} unexpected error: {e}")
        release_lock()
        raise
...
def stream_game(game_id: int, db: Session = Depends(get_db)):
    async def event_stream():
        ...
        def get_ai_pending() -> bool:
            ...
            with ai_move_locks_lock:
                return bool(ai_move_locks.get(game_id, False))
